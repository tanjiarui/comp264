...
CHUNK_SIZE = 1024
HTTP_STATUS = {'OK': ResponseStatus(code=200, message='OK'), 'BAD_REQUEST': ResponseStatus(code=400, message='Bad request'), 'NOT_FOUND': ResponseStatus(code=404, message='Not found'), 'INTERNAL_SERVER_ERROR': ResponseStatus(code=500, message='Internal server error')}
PROTOCOL = 'http'
...
AWS_SERVER_PUBLIC_KEY = 'aws access key'
AWS_SERVER_SECRET_KEY = 'aws secret key'
translate = boto3.client('translate', aws_access_key_id=AWS_SERVER_PUBLIC_KEY, aws_secret_access_key=AWS_SERVER_SECRET_KEY)
polly = boto3.client('polly', aws_access_key_id=AWS_SERVER_PUBLIC_KEY, aws_secret_access_key=AWS_SERVER_SECRET_KEY)
...
class ChunkedHTTPRequestHandler(BaseHTTPRequestHandler):
	...
	def do_GET(self):
		...
		try:
			...
			if path == ROUTE_INDEX:
				response = self.route_index(path)
			elif path == ROUTE_VOICES:
				response = self.route_voices()
	...
	def route_index(self, path):
		"""Handles routing for the application's entry point'"""
		try:
			return ResponseData(status=HTTP_STATUS['OK'], content_type='text_html', data_stream=open(os.path.join(sys.path[0], path[1:]), 'rb'))
		except IOError as err:
			# Couldn't open the stream
			raise HTTPStatusError(HTTP_STATUS['INTERNAL_SERVER_ERROR'], str(err))

	def route_voices(self):
		...
		bytes_data = bytes(json_data, 'utf-8') if sys.version_info >= (3, 0) else bytes(json_data)

		return ResponseData(status=HTTP_STATUS['OK'], content_type='application/json', data_stream=BytesIO(bytes_data))

	def route_read(self, _, query):
		"""Handles routing for reading text (speech synthesis)"""
		# Get the parameters from the query string
		text = self.query_get(query, 'text')
		voiceId = self.query_get(query, 'voiceId')
		source_lan = self.query_get(query, 'language')
		outputFormat = self.query_get(query, 'outputFormat')
		voices = polly.describe_voices()['Voices']
		target_lan = None
		for voice in voices:
			if voice['Id'] == voiceId:
				target_lan = voice['LanguageCode'].split('-')[0]
				target_lan = 'zh' if target_lan == 'cmn' else target_lan
				break
		...
		if len(text) == 0 or len(voiceId) == 0 or outputFormat not in AUDIO_FORMATS:
			raise HTTPStatusError(HTTP_STATUS['BAD_REQUEST'], 'Wrong parameters')
		else:
			try:
				if source_lan != target_lan:
					text = translate.translate_text(Text=text, SourceLanguageCode=source_lan, TargetLanguageCode=target_lan).get('TranslatedText')
			...
			return ResponseData(status=HTTP_STATUS['OK'], content_type=AUDIO_FORMATS[outputFormat], data_stream=response.get('AudioStream'))
...
cli.add_argument('-p', '--port', type=int, metavar='PORT', dest='port', default=8000)
cli.add_argument('--host', type=str, metavar='HOST', dest='host', default='localhost')
arguments = cli.parse_args()
# Create and configure the HTTP server instance
server = ThreadedHTTPServer((arguments.host, arguments.port),ChunkedHTTPRequestHandler)
